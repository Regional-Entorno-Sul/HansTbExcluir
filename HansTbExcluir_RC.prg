...
@ 1,0 say "-----------------------------------------------------------------"
@ 2,0 say "HansTbExcluir.exe - versao RC - 04/02/2026                       "
...
set color to w+/
@ 10,0 say "Argumento escolhido: --hans."

/*
use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
do while .not. eof()
...
set color to g+/
@ 11,0 say "Procura por inconsistencias no numero de lesoes..."
use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
do while .not. eof()
replace tp_not with "x" for nu_lesoes = 0 .or. empty( nu_lesoes ) = .T.
enddo
close

use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
set color to w+/
count to nLesoes for tp_not = "x"
if nLesoes = 0
@ 12,0 say "0 registros com problemas."
close
else
set color to r+/
@ 12,0 say alltrim(str( nLesoes )) + " registros com problemas."
endif
close
set color to g+/

@ 13,0 say "Procura por inconsistencias no numero de nervos afetados..."
use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
do while .not. eof()
replace id_distrit with "x" for nervosafet = 0 .or. empty( nervosafet ) = .T.
enddo
close

use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
set color to w+/
count to nNervos for id_distrit = "x"
if nNervos = 0
@ 14,0 say "0 registros com problemas."
close
else
set color to r+/
@ 14,0 say alltrim(str( nNervos )) + " registros com problemas."
endif
close
set color to g+/

@ 15,0 say "Procura por inconsistencias no exame de baciloscopia..."
use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
do while .not. eof()
replace id_pais with "x" for bacilosco = "2" .or. bacilosco = "3" .or. bacilosco = "9" .or. empty( bacilosco ) = .T.
enddo
close

use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
set color to w+/
count to nBacilo for id_pais = "x"
if nBacilo = 0
@ 16,0 say "0 registros com problemas."
close
else
set color to r+/
@ 16,0 say alltrim(str( nBacilo )) + " registros com problemas."
endif
close
set color to g+/

@ 17,0 say "Excluindo registros que nao satisfazem o criterio..."
use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
do while .not. eof()
replace cep with "x" for tp_not = "x" .and. id_distrit = "x" .and. id_pais = "x" 
enddo
close

use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
set color to w+/
count to nExcluir for cep = "x"
if nExcluir = 0
@ 18,0 say "0 registros com problemas."
close
else
set color to r+/
@ 18,0 say alltrim(str( nExcluir )) + " registros com problemas."
endif
close
set color to g+/

@ 19,0 say "Excluindo os registros sem problemas..."
use "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
delete for cep <> "x"
pack
close

@ 20,0 say "Transferindo registros com problema para um arquivo menor..."
use "c:\HansTbExcluir\for\hans_forma.dbf"
zap
close
copy file "c:\HansTbExcluir\for\hans_forma.dbf" to "c:\HansTbExcluir\out\hans_forma_out.dbf"
use "c:\HansTbExcluir\out\hans_forma_out.dbf"
append from "c:\HansTbExcluir\dbf\hans\hansnet.dbf"
close
set color to w+/
@ 21,0 say "Arquivo final disponivel na subpasta 'out'."
wait
